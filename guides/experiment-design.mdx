...
description: "A practical checklist for PMs, marketers, and engineers running their first few tests — hypotheses, metrics, guardrails, sizing."
---

The mechanics of running a test are straightforward. The hard part is making sure it answers a question you actually care about. This page is a practical checklist — not a statistics textbook.
...
If you can't fill in those three slots, you're not ready to run a test. Sometimes the answer is: do a smaller analysis first to figure out the metric.
...
- **Timely enough to read.** If your goal is a 30-day renewal rate and your test runs two weeks, you have a problem.
...
| `sum` | You want total value (revenue) divided by enrolled units |
...
The unit of randomization is whoever should see one consistent allocation. The most common choices:

| Unit | When |
|------|------|
| `userId` | The most common choice. One user always sees the same allocation. |
...
This is a project-level setting, so it's set once. If you need both user-keyed and company-keyed randomization, create two projects. See [Projects & environments](/concepts/projects-and-environments).
...
Use [policy conditions](/concepts/policies#targeting-conditions) to scope. Be careful: a heavily-targeted policy has fewer [enrolled units](/concepts/terminology#the-three-counts) and will take longer to reach significance.

## Size the measurement

Two things matter:

1. **Minimum detectable effect (MDE).** The smallest change you want to be able to confirm. If you only care about effects of 5% or larger, you need far fewer enrolled units than if you want to detect 1%.
2. **Enrolled units per allocation.** Conversion-rate metrics need enough conversions per allocation. Sum and count metrics need enough enrolled units to overcome variance.

The dashboard will tell you when a measurement has reached significance. If you want to plan ahead, a rough rule for conversion-rate tests:

> enrolled units per allocation ≈ 16 × baseline rate × (1 − baseline rate) / (MDE × baseline rate)²

So if your baseline conversion is 5% and you want to detect a 10% relative lift (`MDE = 0.10`):

> 16 × 0.05 × 0.95 / (0.10 × 0.05)² ≈ 30,400 per allocation ≈ 61,000 total
...
Run tests for at least one full business cycle — usually one or two weeks. Day-of-week and time-of-day effects are real. A test that runs Monday to Wednesday will mis-weight weekend buyers.
...
If you run two policies at once on the same user base, the assignments might interact — one policy's effect depends on the other. Traffical defends against this with [layers](/concepts/layers): policies in different layers are statistically independent by construction.

If you must run interacting tests in the same layer, you have to model them as one policy with the cross-product of allocations (treatment A × control B, treatment A × treatment B, etc.). Two booleans is four allocations. Three is eight. This gets expensive fast — usually layers are the right answer.
...
If you can't write these down, the test can't really lose — every outcome will get rationalized. That's not learning, that's confirmation.
...
- **Was the test underpowered?** If you needed 30k enrolled units per allocation to detect your MDE and you only got 8k, you can't conclude much.
...
- **Running too many tests at once on the same parameter without layers.** Use layers; concurrent policies in different layers don't interact.
- **Changing the policy mid-flight.** Adding an allocation, changing conditions, or changing the goal metric all reset the analysis. Pick carefully, then commit.
...
- **Ignoring novelty effects.** Users react to *change*. A "new" treatment might do better in week 1 and worse in week 4. Run long enough to see past the novelty.
