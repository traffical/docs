...
When prompted, authenticate with `traffical login` (device login) — or paste a management-scoped key (`traffical_sk_...` with `mgmt` scope) from the dashboard's **Settings → API keys**. The CLI uses this to talk to Traffical; it stays on your machine.
