---
title: "Your first A/B test, end-to-end"
...
This walks you all the way through running your first A/B test. By the end you'll have a parameter defined in code, a static A/B policy running in the dashboard, an SDK reading the parameter, track events flowing in, and a results dashboard you can read.
...
- A Traffical account with a project created in the [dashboard](https://dashboard.traffical.io)
...
When prompted, authenticate with `traffical login` (device login) — or paste a management key (`traffical_mk_...`) from the dashboard's **Settings → API keys**. The CLI uses this to talk to Traffical; it stays on your machine.
...
Open `.traffical/config.yaml` and add the parameter we'll test:
...
1. Open **Layers** and pick the base layer (created automatically). Or create a new layer if you want this test in its own pool.
2. Click **New policy**.
3. Configure it:
   - **Kind**: Static
   - **State**: Draft (we'll flip it to Running in a moment)
   - **Allocations**:
     - `control` — bucket range `0–499` — leave `checkout.button.color` unset (it'll use the parameter default)
     - `treatment` — bucket range `500–999` — `checkout.button.color: "#22C55E"` (a green)
4. Save.
...
<Note>
The bucket count for your project is typically 1000. If yours is different (you can check in **Settings → Hashing**), adjust the allocation ranges proportionally.
</Note>
...
4. Aggregation: **conversion rate** (we want to know which colour produces more purchases per enrolled unit).

The dashboard will start computing per-allocation conversion rates as exposure and track events flow in.

## 10. Read the results

After enough traffic accumulates, the policy detail page will show:

- Assignment events per allocation
- Conversion rate per allocation (purchasing enrolled units / enrolled units)
- Confidence interval on each
- A significance indicator when the result becomes statistically meaningful

"Enough traffic" depends on your baseline conversion rate and the size of the effect you're trying to detect — see [Experiment design](/guides/experiment-design#size-the-measurement) for sizing.
...
- **Add more allocations.** A static policy can have any number of them.
...
- **Layer your tests.** Run a second policy on a different parameter at the same time. Put it in a different [layer](/concepts/layers) and they won't contaminate each other.
...
- **"Bucket count looks wrong"** — verify your project's bucket count in **Settings**. Defaults to 1000.
...
- [Experimentation patterns](/guides/canonical-experiments) — patterns for other shapes
