...
### Attribute
A registered, typed [context](#context) key the SDK sends to `decide()` — `plan`, `device_type`, `cart_value`. Registration drives typed condition editors, save-time coercion and validation of condition values, the per-key logging policy, and the generated `TrafficalContext` type. `$`-prefixed keys (`$unit_key`, `$env`, `$country`, the device keys) are system attributes. → [Attributes](/concepts/attributes)
...
### Context
The object you pass to `getParams()` containing the unit key plus any fields used for targeting (locale, plan, device, etc.). Its keys can be registered as [attributes](#attribute). → [Quickstart](/quickstart)
...
### Entry point
A reusable, timestamped milestone that determines when a unit qualifies for measurement, with [qualification rules](#qualification-rules) selecting which fact rows qualify. Attaching one to a measurement narrows the sample from [enrolled units](#enrolled-units) to [qualified units](#qualified-units). → [Terminology](/concepts/terminology#entry-points-and-qualification)
...
### Enrolled units
Units attributed to the allocation of their first assignment; units seen in more than one allocation are excluded. The analysis sample when no [entry point](#entry-point) is active — with one active, the sample is [qualified units](#qualified-units). → [Terminology](/concepts/terminology#the-three-counts)
...
### Qualification rate
Qualified units divided by enrolled units, shown overall and per allocation. → [Terminology](/concepts/terminology#entry-points-and-qualification)

### Qualification rules
Conditions on source-fact dimensions that select which fact rows qualify for an [entry point](#entry-point). → [Terminology](/concepts/terminology#entry-points-and-qualification)

### Qualified units
Enrolled units with a qualifying fact at or after first assignment. The analysis sample when an [entry point](#entry-point) is active. → [Terminology](/concepts/terminology#entry-points-and-qualification)
...
How long after first assignment units reach the [entry point](#entry-point) — the gap between enrollment and qualification, shown as p50/p75/p90. → [Terminology](/concepts/terminology#entry-points-and-qualification)
