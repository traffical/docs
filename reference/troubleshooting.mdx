...
5. **The unit key is missing from the context.** If the evaluation context doesn't include the project's unit key field (for example `userId`), the layer is skipped entirely: no bucket, no policy match, defaults for every parameter in it — and no exposure event is recorded. In multi-entity projects, a layer randomizing on a different entity needs *that* entity's key in the context.
...
A third state, `insufficient_data`, means the allocation has fewer than 30 units — the guardrail hasn't been evaluated yet and surfaces as a health warning, not a breach.
