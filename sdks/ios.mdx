...
The typed getters resolve a single parameter against an inline default and **auto-track an exposure event** the first time an assignment is seen in the session. There is one per value type:
...
When you need the full decision — or want to delay the exposure event until the treatment is actually shown — use `decide(context:defaults:)`. It returns a `TrafficalDecisionResult` with a `decisionId`, the resolved `assignments`, and per-layer `metadata`. Both `decide` and `getParams` take **context first, defaults second**:
...
// ...render the allocation, then record that the user actually saw it:
...
| `trackDecisions` | `Bool` | `true` | Emit decision events alongside exposure events |
...
On first launch the SDK generates an anonymous UUID and persists it in the **Keychain**, so it survives app reinstall (matching what analytics SDKs do). This value fills the project's unit-key slot on every resolution, so bucketing and exposure tracking work before the user logs in.
...
<Warning>
**Bucketing changes at login.** The pre-login (UUID) bucket and the post-login (`user_789`) bucket are computed from different inputs, so a user may cross between allocations at login. This is inherent to having a single unit of randomization per project. If you need assignments stable across login, run the policy on logged-in users only, or call `identify()` before the SDK ever resolves the parameter.
</Warning>

## Device-info provider

Attach a `DeviceInfoProvider` to enrich the evaluation context with device fields — app version, OS, locale, screen size — so you can target policies by them. The bundled `DefaultDeviceInfoProvider` reads app version/build, locale, timezone, OS name/version, and (on UIKit platforms) screen dimensions and device model:
...
The provider's fields are merged into the context on every resolution, so a policy condition like `appBuildNumber gte 500` or `$locale eq "en_US"` can target them. Implement the `DeviceInfoProvider` protocol yourself to add or override fields.

`DefaultDeviceInfoProvider` emits two families of keys: the `$`-prefixed [system attributes](/concepts/attributes#system-attributes) shared by every Traffical SDK — registered in every project, so you can target them straight from the condition editor — and the original un-prefixed keys, kept for compatibility with existing conditions. Use the `$` keys in new conditions.

| Key | Type | Value |
|-----|------|-------|
| `$app_version` | string | `CFBundleShortVersionString` (semver-style; stays a string) |
| `$os` | enum | `ios` on iOS and tvOS, `macos` on macOS, `other` elsewhere (watchOS included) |
| `$os_version` | string | `major.minor.patch` from `ProcessInfo.operatingSystemVersion` |
| `$device_type` | enum | UIKit platforms: phone → `mobile`, pad → `tablet`, every other idiom → `desktop`; non-UIKit platforms → `desktop` |
| `$device_model` | string | `UIDevice.current.model` (UIKit platforms only) |
| `$locale`, `$timezone` | string | `Locale.current.identifier`, `TimeZone.current.identifier` |
| `appVersion`, `appBuildNumber`, `locale`, `timezone`, `osName`, `osVersion`, `screenWidth`, `screenHeight`, `deviceModel` | | Compatibility keys. `appBuildNumber` is emitted as a **number** when the build is an integer, so `appBuildNumber gte 500` matches under strict typing; `osName` keeps the precise platform (`watchos`) where `$os` maps to `other`. |
