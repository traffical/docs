...
When several parameters make up **one treatment that must be measured together**, model them as a single `json` object-flag instead. One flag → one `variant` → one exposure, and it's the ITT-parity option (the whole bundle is decided and exposed as a unit):
