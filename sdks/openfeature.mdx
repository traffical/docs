...
<Note>
  **Resolve is a decision, not an exposure.** Reading a flag value records an intent-to-treat (ITT) decision event. It does **not** record that the user actually saw the treatment. You **must** signal exposure explicitly (or opt into `exposureOnResolve`) — otherwise your primary metrics, SRM health, and optimization stay empty. See [Signalling exposure](#signalling-exposure).
</Note>
...
| `exposureOnResolve` | `boolean` | `false` | Fire an exposure event automatically on every resolve. Collapses TOT ≈ ITT — see [Signalling exposure](#signalling-exposure). |
...
A **resolve records a decision event** (the intent-to-treat / ITT signal) — it means "this unit *would* get this allocation." A **true exposure event** records that the unit actually experienced its allocation — treatment-on-the-treated (TOT) — and it's a separate, explicit signal. The provider never turns a resolve into an exposure automatically (doing so would erase the ITT-vs-treated distinction), so you fire exposure at render time via the reserved tracking event:
...
<Warning>
  **Missing exposure events leave your measurement empty.** Traffical's default primary metric source is **SDK Exposures (TOT — as-treated)**, and exposure events also gate the **SRM health check** and **bandit/optimization training**. A drop-in integration that resolves flags but never fires `$traffical.exposure` will silently record decision events with:
  ...
  The provider watches for this: if it sees decision events but no exposure events past a threshold, it emits a one-time warning and a `PROVIDER_ERROR` telling you to fire `$traffical.exposure` or set `exposureOnResolve`.
</Warning>

### `exposureOnResolve` — the zero-friction escape hatch

If you can't (or don't want to) thread an explicit render-time signal, set `exposureOnResolve: true`. The resolver then fires the exposure event on the decision it just made — same `decisionId`, correct propensity and config version, filtered to the owning layer:
...
| `traffical.allocationId` / `traffical.allocationKey` | The allocation the unit landed in. |
...
Metadata reflects the flag's **owning layer** — never a sibling policy's layer that the unit happens to be bucketed into. On web, bandit internals (`propensity`, `modelVersion`) are gated out because `flagMetadata` is visible in browser devtools.
...
The provider always sets the `unitKey` from the request's `targetingKey` (server) or bound identity (web) — if it can't derive one it fails loud rather than emitting an unjoinable event. Reward metrics join to the first exposure event temporally on `unit_key`, so **a correct unit key is all reward correctness requires** — carry `flagKey` and other fields as metadata, never inside the numeric `value`.

## Co-measured parameters: the object-flag pattern

By default, one parameter is one flag, each with its own exposure event. Deterministic bucketing keeps a unit consistent across flags, so this is the natural model.

When several parameters make up **one treatment that must be measured together**, model them as a single `json` object-flag instead. One flag → one `variant` → one exposure event, and it's the ITT-parity option (the whole bundle is decided and exposed as a unit):
...
// One flag, one exposure event for the whole co-measured allocation:
...
Reach for the object-flag when parameters are interdependent (they only make sense evaluated together) or when tests must share one ITT population. Keep separate flags when parameters vary independently.

## Measurement fidelity

A few honest properties worth knowing:

- **ITT is reshaped, not inflated.** Denominators are per-unit, so resolve-without-render and repeat resolves don't inflate ITT. But because sibling `attributionOnly` layers are dropped from the ITT source, per-key resolution records a co-running policy's ITT only when *its own* flag is resolved. Batched per-request decisions (server) and the object-flag pattern restore native parity when you need co-measured populations.
- **Exposure events gate the primary path** (TOT, SRM, optimization) — see the warning above.
- **Reward metrics need only `unit_key`;** the attribution array is auxiliary (dashboard breakdowns).
- **Propensity is captured faithfully** on the exposure event and is future-enabling (off-policy evaluation), not a present correctness requirement. Its capture depends on exposure events firing and on never re-deciding.
...
<CardGroup cols={2}>
  ...
  <Card title="Events & metrics" icon="chart-line" href="/concepts/events-and-metrics">
    How exposure events and rewards become metrics.
  </Card>
  ...
  <Card title="Optimization" icon="wand-magic-sparkles" href="/experimentation/optimization">
    How exposure events train the bandit.
  </Card>
</CardGroup>
