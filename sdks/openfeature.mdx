...
The providers are thin translation layers over the native clients, so they inherit the same SHA-256 bucketing, layered resolution, and contextual-bandit scoring as every other Traffical SDK — a given unit buckets identically whether you resolve through OpenFeature or the native client.
...
const traffical = createTrafficalClient({
  orgId: "org_acme",
  projectId: "proj_marketplace",
  env: "production",
  clientKey: process.env.TRAFFICAL_CLIENT_KEY!,
});
...
| `reasonMapping` | `Record<string, string>` | — | Cosmetically relabel the OpenFeature `reason` string. Never changes which events fire. |
...
<Note>
  The server provider supports `evaluationMode: "bundle"` clients (local, per-request context). A `"server"`-mode client resolves against a background-polled snapshot with an empty context and ignores per-request `targetingKey`, so it can't do per-request targeting — the provider warns if you construct it with one.
</Note>
