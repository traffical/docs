...
| `apiKey` | `string` | required | Server-side SDK key (`traffical_sk_...`) |
...
`getParams` returns just the resolved values. `decide` returns a `DecisionResult` (assignments + a `decisionId` + metadata) — useful when you want to record an exposure event only when the user actually sees the treatment, or pass the decision ID downstream.
...
If you're running an adaptive policy, the optimizer uses these events as reward signals to learn which allocations perform best.
