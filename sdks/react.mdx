...
| `trackExposure` | `() => void` | Manually emit an exposure event (for `"decision"` mode) |
| `flushEvents` | `() => Promise<void>` | Force flush of pending events |

### Tracking modes

```tsx
useTraffical({ defaults, tracking: "full" });      // default — automatic decision + exposure events
...
Use a publishable SDK key (`traffical_pk_...`) in the client provider and a server-side SDK key (`traffical_sk_...`) for the server-side bundle fetch, kept in a private environment variable. See the [SSR patterns](/sdks/ssr) page for the full setup, including the Pages Router.
...
| `apiKey` | `string` | required | Publishable Publishable SDK key (`traffical_pk_...`) — safe in a client bundle |
...
| `trackDecisions` | `boolean` | `true` | Emit decision events alongside exposure events |
