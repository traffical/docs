...
<Note>
  This is the right mode for **continuous monitoring** and for automated systems that evaluate results repeatedly. There's no peeking penalty to budget and no pre-committed sample size to wait out.
</Note>
...
The width is tuned by a **target sample size**: the sequence is at its tightest around the sample size you expect the measurement to reach, and looser far from it. Traffical derives that target from the policy's power analysis — the sample required at your target MDE and power — and falls back to a sensible default when there isn't one. Sample sizes here are counts of [enrolled units](/concepts/terminology#the-three-counts) — or [qualified units](/concepts/terminology#entry-points-and-qualification), when an entry point gates the sample.

<Tabs>
  <Tab title="Early in the run">
...
- **Project default** — a default for every policy in the project, set in the project's analysis settings.
...
<Note>
  Sequential testing changes how results are *read*, not what data is collected — so you can switch a running policy between fixed-horizon and sequential and the numbers recompute from the same aggregates.
</Note>
...
<Tip>
  Rule of thumb: if anyone — a person or an automated process — will look at the results more than once before the run ends, use sequential testing.
</Tip>

For the underlying math — the half-width formula, the tuning parameter, and why there's no sequential p-value — see the [technical companion](/statistics/sequential-testing-technical).
