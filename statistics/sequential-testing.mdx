...
The width is tuned by a **target sample size**: the sequence is at its tightest around the sample size you expect the experiment to reach, and looser far from it. Traffical derives that target from your experiment's power analysis — the sample required at your target MDE and power — and falls back to a sensible default when there isn't one.
