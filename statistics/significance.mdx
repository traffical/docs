...
- **Standard error** — how much that lift would wobble from sampling alone, built from each allocation's variance and sample size — its count of [enrolled units](/concepts/terminology#the-three-counts), or [qualified units](/concepts/terminology#entry-points-and-qualification) when an entry point gates the sample.
...
- **p-value** — the probability of seeing a lift this large if the allocations were truly identical.
...
<Tabs>
  <Tab title="Binary / proportion">
    **Conversion rate** — did the unit convert or not. The value is the share of enrolled units that converted, and the variance is `p(1 − p)`. Use it for signups, checkouts, click-through, activation.
  </Tab>
  <Tab title="Mean">
...
- To **reach significance sooner**, add a pre-assignment covariate with [CUPED](/statistics/cuped).
