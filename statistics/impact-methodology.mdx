...
Each shipped change contributes a **projected daily impact**: the per-unit effect it measured, converted to metric units per day, scaled to a full rollout.

```
projected / day  =  (per-unit effect)  ×  (treatment units ÷ window days)  ÷  (treatment share)
```

- The **per-unit effect** is the absolute difference between the shipped allocation's mean and control's, exactly as measured.
- **Treatment units ÷ window days** turns that into units per day: how many [enrolled units](/concepts/terminology#the-three-counts) per day the winning allocation accrued during the analysis window.
