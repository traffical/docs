...
Some of the most important metrics aren't a simple average over units. **Revenue per session**, **average order value**, **items per order** — each is a ratio where *both* the top and the bottom change from unit to unit. Ratio metrics measure these correctly.

## Why a ratio is different

Take revenue per session. If you divided total revenue by total sessions and stopped there, you'd have a number — but no honest sense of how uncertain it is, because the denominator (sessions) is itself random and varies between units.
...
<Note>
  Use a ratio metric whenever the quantity you care about is naturally "X per Y" and Y isn't fixed — per session, per order, per visit. If the denominator is simply the number of [enrolled units](/concepts/terminology#the-three-counts), an [average or conversion-rate metric](/statistics/significance) is the simpler choice.
</Note>
...
<Tabs>
  <Tab title="Revenue per session">
    Numerator: purchase revenue. Denominator: sessions. Answers "did the treatment change how much each session is worth?" — independent of whether it also changed the number of sessions.
  </Tab>
