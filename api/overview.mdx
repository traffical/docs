...
| Prefix | Type | Scopes | Use case |
|--------|------|--------|----------|
| `sk_...` | Server key | `sdk:read`, `sdk:write` | Server-side SDKs and direct API calls |
| `pk_...` | Public key | `sdk:read`, `sdk:write` (events only) | Browser SDKs — safe to ship in client code |

- **`sdk:read`** — fetch bundles, request resolutions
- **`sdk:write`** — send events

Server keys can do everything. Public keys are safe to include in client-side JavaScript: they can fetch the project's bundle and send events, but cannot modify configuration in any way.
