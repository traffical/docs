...
When any matched policy logs context fields (its own context fields plus attributes with logging `always`, minus `never`), the response also includes `metadata.filteredContext` containing just those fields — the same set written to the decision event.
