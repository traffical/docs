---
title: "A/B testing"
description: "Run a static A/B test end-to-end — define parameters, create a policy, implement in code, measure results."
---
...
- **State:** running
- **Allocations:**
  - `control` — buckets `0–499` — `checkout.button.color: "#1E6EFB"`
  - `treatment` — buckets `500–999` — `checkout.button.color: "#22C55E"`

That's a 50/50 split assuming a `bucketCount` of 1000.
...
Users outside the segment get the parameter's default value. They aren't part of the enrolled units (units attributed to the allocation of their first assignment; units seen in more than one allocation are excluded) — they don't dilute the result.

See [Policies](/concepts/policies#targeting-conditions) for the full list of operators.

## Picking the unit key

Bucketing is based on your project's [unit key](/concepts/projects-and-environments#the-unit-key). For most policies this is `userId`. For B2B / multi-tenant SaaS, you usually want `companyId` instead — every user in a company sees the same allocation, which is what your customers will expect. If you need both kinds of randomization, create two projects.

## Concluding the test

When you've collected enough data:

1. **Pick the winner.** Update the parameter's default value to the winning allocation's value.
...
If two allocations aren't enough, just add more to the policy. Three, five, ten — the math still works as long as you've sized the measurement for the smaller enrolled units per allocation. For continuous optimization across many allocations, use an [adaptive policy](/experimentation/optimization) instead.
...
<CardGroup cols={2}>
  ...
  <Card title="Experiment design" icon="lightbulb" href="/guides/experiment-design">
    Hypotheses, metrics, guardrails, sizing.
  </Card>
