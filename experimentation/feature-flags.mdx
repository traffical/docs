...
Flip the default in the dashboard (or push `default: true` via the CLI) and every user gets the new value on the next bundle refresh. No test, no policy — just a remote-controlled boolean.
...
## Flags as the entry point to experimentation
...
If you find yourself adding more booleans to control treatment behaviour (`show_new_layout`, `use_new_copy`, `apply_new_discount` — all in the same policy), you're hitting the wall of the boolean model. Switch to typed parameters:
...
One policy can override all three together. One A/B test compares "classic" against "v2" without three correlated booleans masquerading as one test.
