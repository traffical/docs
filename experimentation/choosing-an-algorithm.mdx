...
Adaptive policies optimize the goal event as a per-user **count** by default. The contextual bandit dialog asks for the goal type explicitly; for other algorithms, count is the default and fits most engagement-style goals.
...
For a **Contextual Bandit** you'll also choose the **Goal Type** and list the **Context Fields** the SDK should send for personalization (these must be allow-listed for logging — see [Optimization](/experimentation/optimization#context-logging-allowlist)).
