...
DevTools is independent of the [visual editor](/tools/visual-editor): the visual editor authors tests, DevTools inspects them.
...
- **Override a parameter** — force a value locally for the current browser session. Useful for testing the treatment allocation without being assigned to it.
...
- Diagnosing why a user is in the wrong allocation (wrong bucket? wrong context? policy paused?)
- Testing a policy's treatment allocation without being assigned to it
- Walking through a test with a colleague who isn't on your team
