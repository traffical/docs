...
```
.traffical/
├── config.yaml             # Parameters, attributes, events, property groups
...
|------|---------|----------------|
| `config.yaml` | Parameter, attribute, event, and property-group definitions | Yes |
...
The main file. It declares parameters, context attributes, events, and reusable property groups. Everything declared here becomes **synced** when you `traffical push` — its definition turns read-only in the dashboard to prevent drift between your repo and the platform.
...
| `attributes` | No | Context [attribute](#attributes) definitions. |
...
[Attributes](/concepts/attributes) are the typed registry of context keys your SDKs pass to `decide()`. Registering them gives the dashboard typed condition editors, lets the platform validate and coerce policy targeting against the declared type, and controls what the SDK logs with decision and exposure events.
...
| `format` | No | Narrows a string: `enum` (requires `values`), `semver`, `country`, or `url`. |
| `values` | No | Allowed values — a list of strings, or a map of value → description. Required for `enum`; an optional subset of ISO 3166-1 alpha-2 codes for `country`. |
| `range` | No | `[min, max]` bounds for `number` attributes. |
| `label` | No | Display name in the dashboard. |
| `description` | No | Free-form text. |
| `identifier` | No | Marks a key that may serve as a unit or entity key. Defaults `logging` to `never`. |
| `logging` | No | `never`, `allowed` (default), or `always` — whether the value is written to decision and exposure events. See [logging](/concepts/attributes#logging). |
| `breakdown` | No | Offer the attribute as a results breakdown dimension (stored now; breakdowns ship later). |
| `encoding` | No | Training hints for contextual policies: `{ binning: none \| quantile, topK: <int> }`. |

Keys must match `^[A-Za-z_][A-Za-z0-9_.]*$` and be at most 128 characters. System attributes (`$unit_key` and the other `$`-prefixed keys) are managed by Traffical: `pull` never writes them and `push` rejects them.

`traffical push` sends attributes **first** — before parameters, property groups, events, and metrics — so policies validated later in the run already see the registry. A file with no `attributes:` block leaves the project's attributes untouched, including with `--prune`.
...
<Tip>
Mark properties you'll want to break measured results down by with `dimension: true`, and numeric properties you'll want to build metrics on with `measure: true`. Both feed the [warehouse-native](/concepts/warehouse-native) pipeline.
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="Attributes" icon="tags" href="/concepts/attributes">
    Typed context keys for targeting, logging, and generated types.
  </Card>
