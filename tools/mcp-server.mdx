...
| `layers:write` | create layers from templates (`create_layer`) — configuration authoring, governed by the same rules as the dashboard's layer dialog |
...
| `list_attributes` | `attributes:read` | Registered context [attributes](/concepts/attributes) with type, format, and legal values — call it before authoring a condition, since operators and value types are validated against the registry |
...
| `create_layer` | `layers:write` | Create a **new layer from a template** in one atomic call: the layer, its parameters, and its first policies. Templates are `feature_flag` (one parameter plus an **On** policy — a [feature flag](/experimentation/feature-flags)), `standard` (one 100% draft policy), and `url_redirect`. `startNow` is honored only in write mode `execute` and through the approval gate; otherwise the policy is created as a draft and `start` says why. Returns the layer, parameter, and policy ids and a console link. |
...
| Mode | Effect |
|------|--------|
| `off` | Transitions rejected outright — `create_change` still authors the draft, but its `start` leg is refused; `create_layer` is refused |
