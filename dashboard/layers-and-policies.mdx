...
The **Layers** page is where every A/B test, rollout, and feature flag lives. Layers keep concurrent policies cleanly separated; policies decide who sees what within a layer. A [feature flag](/experimentation/feature-flags) is a layer with one parameter. This page walks the dashboard surfaces — for the underlying model, see [Layers](/concepts/layers) and [Policies](/concepts/policies).
...
- **Feature flag** — one parameter and an **On** policy serving its on value to every bucket. Asks for the flag name, the key (a parameter group plus a `snake_case` suffix), the type, optional allowed values, the off and on values, and whether to turn it on right away.
...
The **New layer** button creates a Standard layer; the menu beside it opens the same dialog on **Feature flag** or **URL redirect**. The footer states what will be created, and everything is created in one step: a failed step never leaves a half-built layer behind. Turning a flag on from the dialog is a policy start and follows your organization's [approval rules](/governance/approvals-and-autonomy); when a rule applies, the policy is created as a draft and the approval request is raised for you.

Randomization unit and analysis signal overrides sit behind **Description, randomization unit, analysis signal** and default to the project's settings.
...
- **KPI band** — four numbers: lifetime **[Assignment events](/concepts/terminology#assignment-events)** (serving volume — decision and exposure rows, repeats included), **Traffic** (the share of the layer's buckets the policy's allocations cover), **Lift vs control**, and **Health**. With a primary metric attached, the lift KPI shows the *measured* lift of the best challenger; it stays uncolored and flagged "not yet significant" until the result is conclusive, and the sign of a win follows the metric's desired direction. Without a metric it falls back to the optimizer's estimate.
...
- When an [entry point](/concepts/terminology#entry-points-and-qualification) is attached, sizing and projections run on **qualified units** — enrolled units that reached the entry point — with the planned qualification rate shown alongside
