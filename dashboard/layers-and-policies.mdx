...
The **Layers** page is where every experiment, rollout, and feature-flag policy lives. Layers keep concurrent experiments cleanly separated; policies decide who sees what within a layer. This page walks the dashboard surfaces — for the underlying model, see [Layers](/concepts/layers) and [Policies](/concepts/policies).
...
<Frame caption="Create policy">
  <img src="/images/placeholders/create-policy-modal.png" alt="Create Policy modal with name, key, kind, goal, and traffic allocation fields" />
</Frame>
...
| Tab | What it answers |
|-----|-----------------|
| **Overview** | Where does the experiment stand right now? |
| **Setup** | How is it configured — splits, parameters, measurement targets? |
| **Measurement** | What do the measured results say, and how far along is the experiment? |
...
4. **Still learning.** No significant winner yet — the banner names the trending variant, if any, and tells you to keep running.
...
- **Configuration** — algorithm, goal, minimum exposures before traffic shifts, and context fields for contextual policies. Editable while in draft.
...
The **results panel** lists every attached metric with per-variant point estimates, confidence intervals, and significance — see [Significance](/statistics/significance) for how the numbers are computed and [Metrics](/dashboard/metrics) for attaching them.
...
Adaptive policies also get their model behavior here: the optimization status panel, a convergence readout (exploring → learning → converging → converged), and the per-arm learning state. The converged advice applies the same standard of evidence as the verdict banner — once the optimizer converges, the tab only suggests completing the policy if the leader's measured lift is significant; otherwise it says to keep running until it reaches significance. Per-entity policies show how many entities are being independently optimized and which variant wins across them; contextual policies show training status and per-context model behavior instead.

### Measurement progress

The **measurement progress card** tracks how close the experiment is to its target minimum detectable effect:
