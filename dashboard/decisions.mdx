...
It's both a debugging tool ("why did this policy pause itself last night?") and a compliance tool ("show every change to this policy in the last quarter").

<Note>
Despite the shared word, the decision log is unrelated to [decision events](/concepts/terminology#decision-log-vs-decision-events). Decision events are runtime assignment events the SDK emits; the decision log holds governance decision records — approvals, ramps, promotions.
</Note>
