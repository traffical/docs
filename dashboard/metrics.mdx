...
On **Conversion** and **Sum** metrics you can enable a pre-exposure covariate: the metric's own value over a lookback window (default 14 days) before each unit's first exposure. It shrinks confidence intervals and takes effect only when [CUPED](/statistics/cuped) is also enabled on the policy or project. CUPED isn't available for count or ratio metrics.
