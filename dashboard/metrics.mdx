...
- **Window** (optional) — a per-component attribution window, counting only value from *X* to *Y* days after first assignment.
...
- **Timeframe** limits attribution to the first *N* days after first assignment.
...
On **Conversion** and **Sum** metrics you can enable a pre-assignment covariate: the metric's own value over a lookback window (default 14 days) before each unit's first assignment. It shrinks confidence intervals and takes effect only when [CUPED](/statistics/cuped) is also enabled on the policy or project. CUPED isn't available for count or ratio metrics.
...
- becomes **read-only**, so the definition can't drift under running policies, and
...
Uncertifying is deliberate: it asks you to type `UNCERTIFY`, since colleagues and running policies may rely on the definition. Once uncertified, the metric becomes editable again.
...
- **Delete** is only allowed if the metric has never shipped in a non-draft policy. Once it has, the definition is part of the evidence record for those policies — the server refuses and points you to archive instead.
