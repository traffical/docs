...
- **CUPED variance reduction** — use data from before each unit's first assignment to shrink confidence intervals and reach significance sooner, without biasing results. Off by default. See [Variance reduction (CUPED)](/statistics/cuped).
...
**Server-side context logging** decides what the server-side resolution endpoint records for context keys that are not registered as [attributes](/concepts/attributes#server-side-resolution) and that no matched policy lists. **Names only** (the default) writes the key with no value, enough for the key to appear under **Seen in traffic, not registered**. **Keys and values** also writes the values, which lets discovery infer types and show common values; use it while onboarding and switch back once you have registered what you need, because raw context then reaches your warehouse. Attributes set to `logging: never` are excluded either way, and SDKs evaluating a bundle locally are unaffected.
