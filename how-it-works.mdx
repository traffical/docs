...
When a parameter is resolved, the SDK emits an **exposure event** recording that the user experienced their allocation in each layer they touched. When your code calls `track()`, the SDK emits a **track event** that records a user outcome (a purchase, a click, a signup).
