...
**Independent across layers.** Each layer mixes its own ID into the hash, so a user's bucket in one layer carries no information about its bucket in another. This is what lets concurrent policies coexist without contaminating each other.
...
When a parameter is resolved, the SDK emits a **decision event** recording that an allocation was resolved for the user — the intent-to-treat (ITT) signal, whether or not they saw it. When your code reads a parameter value, the SDK emits an **exposure event** recording that the user actually experienced their allocation in each layer they touched — the treatment-on-the-treated (TOT) signal. When your code calls `track()`, the SDK emits a **track event** that records a user outcome (a purchase, a click, a signup).

All of them are batched in memory and shipped in the background. The Traffical service uses them for two things:
...
<CardGroup cols={2}>
  ...
  <Card title="Policies" icon="route" href="/concepts/policies">
    How allocations and conditions determine who sees what.
  </Card>
