...
SDKs resolve parameters locally in sub-milliseconds. The optimizer learns from user behaviour and continuously refines the traffic split. Metrics are computed either from Traffical-native events or directly from your own data warehouse.

## Key capabilities

- **Parameter-first.** Typed values (string, number, boolean, JSON) with defaults and constraints. A/B tests, rollouts, and flags are all policies on top of parameters.
- **Layered experimentation.** Mutual exclusivity within a layer; orthogonal bucketing across layers. Run dozens of concurrent policies without them contaminating each other.
...
- **Honest statistics.** Sequential (anytime-valid) testing, [CUPED](/statistics/cuped) variance reduction, a [measurement progress](/statistics/measurement-progress) readout that estimates how much longer a measurement needs, and a deliberately conservative [program-level impact estimate](/statistics/impact-methodology).
...
- **Visual editor and DevTools.** Author no-code UI tests via a bookmarklet; inspect SDK state and override parameters live in any browser.

## Who is Traffical for?

Engineering teams who want to:

- Run A/B tests without adding latency to their application
...
- Run many concurrent policies safely
