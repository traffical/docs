...
- A Traffical account with a [project](/concepts/projects-and-environments) created in the [dashboard](https://dashboard.traffical.io)
- A **publishable SDK key** (`traffical_pk_...`) for client-side code, or a **server-side SDK key** (`traffical_sk_...`) for a backend — from your organization's **API Keys** page (in the sidebar when viewing your organization, or from the account menu)
- Node.js 18+
...
<Steps>
  ...
  <Step title="Resolve a parameter">
    ...
    console.log(params["checkout.button.color"]); // "#1E6EFB" or an allocation's override
    console.log(params["checkout.headline"]);     // "Complete your order" or an allocation's override
    ```
