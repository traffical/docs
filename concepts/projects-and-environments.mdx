...
API keys are scoped to a project and environment. Two kinds:

| Prefix | Type | Where to use |
|--------|------|-------------|
| `sk_...` | **Server key** | Server-side SDKs (`@traffical/node`), CLI, server-to-server API calls. Keep secret. |
| `pk_...` | **Public key** | Browser SDKs (`@traffical/js-client`, `@traffical/react`, `@traffical/svelte`) and React Native. Safe to expose in client code. |

Server keys can fetch the bundle and send events. Public keys can fetch the bundle and send events but cannot modify configuration. Create and manage keys in **Settings → API keys** in the dashboard.
...
For browser SDKs, you often don't have a `userId` yet — the user hasn't logged in. The browser SDK auto-generates a stable ID (a UUID) on first visit, stores it in `localStorage` with a cookie fallback, and uses it as the unit key. When the user logs in, call `identify(userId)` and the SDK switches over. Assignments may change at that point — anonymous bucketing is ephemeral, logged-in bucketing is the real experiment assignment.
