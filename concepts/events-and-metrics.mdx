...
When a track event has no `decisionId`, the platform falls back to time-window attribution: events are linked to the most recent decision for the same unit key, within a configurable attribution window.
