...
Events are how Traffical sees what happened in your application: which variants users were exposed to and what they did afterwards. Events power metric dashboards, statistical significance calculations, and the optimizer that learns from user behaviour. Events are the raw material; [metrics](/dashboard/metrics) aggregate them into the numbers experiments are judged on, computed with the methods in the [statistics overview](/statistics/overview).
