...
description: "Compute metrics directly from your own data warehouse. Define assignments and facts as SQL — Traffical aggregates and runs significance tests."
---
...
|------------|-------------------|
| **Entity** | The unit of randomization (User, Company, Device). What kind of thing gets assigned to an allocation. |
...
The entity's key type must match the project's unit key.

### Assignment definitions

An assignment definition is the SQL that produces the mapping between an entity and the [allocation](/concepts/terminology#allocations-and-traffic-splits) it was assigned to. This is what tells Traffical "user 123 saw `treatment` of `pricing_test` at 10:30am".
...
An assignment definition also declares its **Signal** — the [analysis signal](/concepts/terminology#analysis-signal) its rows carry: decision events (intent-to-treat) or exposure events (treatment-on-the-treated). If your source holds both kinds, map a `source_type` column and declare the signal as mixed; each row then says which kind it is.

If you're using a Traffical SDK to do the assigning, you get the same effect for free — the SDK ships assignment events to Traffical and the platform manages the assignment table for you. You only need to write an assignment definition if assignments come from somewhere else.

The managed assignment table (and the BYO-warehouse assignment-logging path) also carries a few optional columns per assignment: `bucket` (the deterministic bucket the unit hashed into), `propensity` (the probability the SDK gave the chosen allocation at decision time — a fixed split's share, or a bandit's sampled probability), `model_version` (for contextual policies, the coefficient set that scored the decision), and `config_version` (the config bundle the SDK evaluated against). These power honest analysis of adaptive traffic and off-policy evaluation; they're additive and safe to ignore if you don't need them.
...
|-------------|-----------|-------------|--------|
| `conversion_rate` | Enrolled units with at least one fact event | Enrolled units | Proportion |
| `sum` | Sum of `valueColumn` across fact events | Enrolled units | Per-entity average |
| `count` | Count of fact events | Enrolled units | Per-entity rate |

The denominator is always the [enrolled units](/concepts/terminology#the-three-counts) of the analysis: units attributed to the allocation of their first assignment, with units seen in more than one allocation excluded.

## The pipeline

When a metric is configured, Traffical runs a pipeline on a schedule:

1. Reads assignments (from your warehouse SQL, or from Traffical's assignment events if SDK-managed)
...
```mermaid
flowchart LR
    A["Assignment source<br/>SDK assignment events<br/>or warehouse SQL"] --> P[Pipeline]
...
- SDK-managed assignments for new policies (no SQL to write)
...
If you want your SDK events to land in your warehouse too — for ad-hoc analysis, ML pipelines, or compliance — Traffical can sync decision, exposure, and track events into a table you specify. This is configured per project; once it's on, every event ingested also lands in your warehouse on a regular cadence.
