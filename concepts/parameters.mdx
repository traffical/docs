...
A **parameter** is a typed configuration value with a default. Parameters are the atomic unit in Traffical: A/B tests, feature flags, rollouts, and adaptive optimization are all [policies](/concepts/policies) that override parameter values for selected users.
...
- [Layers](/concepts/layers) — every parameter belongs to exactly one layer. Layers exist so concurrent policies don't interfere.
