...
The rule is the same wherever the decision is made. An SDK evaluating locally writes exactly the keys on the matched policies' allow-lists — a key a policy lists is logged whether or not it is registered. Server-side resolution follows the allow-lists too, and additionally reports unregistered keys by name; see below.

#### Server-side resolution

When you resolve through [`/v1/resolve`](/api/post-resolve), the decision event carries only the context the matched policies' allow-lists admit: each policy's context fields, plus every attribute with `logging: always`, minus every attribute with `logging: never`. The raw request context is never written. `$country`, which Traffical derives from the request for targeting, is logged only under the same rule — a policy lists it, or it is set to `always`.

Unregistered keys in the request are handled by the **Server-side context logging** setting under **Settings → SDK**:

| Setting | Effect |
|---|---|
| **Names only** (default) | Every unregistered key no policy lists is written with a `null` value. The key shows up under **Seen in traffic, not registered**; its values never leave the request. |
| **Keys and values** | Unregistered keys are written with their values, so discovery can infer types and show common values. Meant for onboarding: raw context reaches your warehouse, so switch back to **Names only** once you have registered what you need. |

Attributes with `logging: never` are excluded in both modes. The setting applies to server-side resolution only; SDKs evaluating a bundle locally are unaffected.
...
The derived value is available to conditions regardless of its `logging` setting. It is written to the decision event only when a matched policy lists it or the attribute is set to `always` — see [server-side resolution](#server-side-resolution).
...
Keys that reached the warehouse without values — the default for unregistered keys on [server-side resolution](#server-side-resolution) — are marked **names only**. There is nothing to infer a type from, so **Register…** prefills the type as `string`; change it in the sheet if the key carries numbers, booleans, or timestamps.
